...
**Implementation status, 2026-09-25:** a native Kotlin/Jetpack Compose debug app builds. It includes Garage, Design, PIDs, and Device screens, versioned local vehicle profiles with separate drafts, five simulated round renderers, searchable example PID catalog, Mode 01 decoder lab, coolant threshold preview, a public BLE capability reader, and a bounded authenticated write for the gauge's built-in reading selection. No adapter, live telemetry, firmware update, code clearing, or full device configuration transfer is implemented. The [browser prototype](../design/prototype/index.html) remains a separate design review artifact.
...
The app opens in demo mode. Every synthetic value is labeled. Each local vehicle profile keeps its own layout, example PID, source, and coolant warning/critical, hysteresis, and dwell values. These alert settings are local drafts only. The version 1 profile store migrates the earlier single draft and stops editing if stored data has an unreadable or future format. New profile IDs conform to the device configuration ID pattern; older local IDs are preserved until an explicit migration is designed. Profile names are user labels, not vehicle capability evidence. Full Apply remains disabled and now lists concrete blockers: draft validity, gauge write capability, source-specific vehicle evidence, and the missing versioned transfer. "Find nearby gauge" requests the platform BLE permissions, filters for the eGauge service UUID, connects, reads capabilities, and closes the discovery link. The app retains that discovered device for the current app session, so the control action does not start a new scan that might select another gauge. A later device picker must make gauge identity explicit when multiple gauges are nearby. When `quickSelect` is advertised, Device offers **Set preview reading on gauge**. Android explicitly starts OS bonding when needed, then a two-byte authenticated write selects one of five built-in readings. The app reports success only after an authenticated state readback. This new flow needs an on-phone pairing review. Android 10/11 request location for BLE scanning; Android 12+ request Nearby Devices. A missing gauge, disabled Bluetooth, denied permission, timeout, or unsupported protocol remains an explicit state.
