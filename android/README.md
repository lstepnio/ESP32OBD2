...
The Device screen can choose a `.egauge-dev-update` ZIP from Android's system document picker. The importer requires exactly `metadata.json` and `firmware.bin`, bounds the image to the 3 MiB OTA slot, checks board tag, protocol, SHA-256 and ESP app descriptor, and verifies the P-256 signature against the pinned development public key. The development bundle can be made with `tools/sign_dev_update.py --bundle`. **Install development update** sends sequential chunks over the authenticated owner link, checks accepted offsets, asks the gauge to verify and activate, then reads the running partition, valid state and ELF hash after reboot. [One live update](../docs/development/update-live-validation.md) completed. The private development key stays outside Git and the APK. This foreground development path still needs interruption, changed-build and rollback exercise before the public OTA flag can become true.
