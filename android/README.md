...
The current source packages in `:app` are model/state, versioned local profile storage, BLE capability client, PID decoder, and Compose UI. This avoids premature Gradle module overhead while their contracts settle. The package/module plan below remains the target for the authenticated configuration slice. SharedPreferences holds only small non-secret drafts; migrate to DataStore/Room with further schema migration before large catalogs and sourced discovery evidence. The public BLE read closes immediately after the capability response. Owner association, identity/bond storage, serialized GATT operations, authenticated control, atomic apply, and OTA are still required. A Pixel 10 Pro render and BLE check passed; broader layout/accessibility signoff is still needed.
