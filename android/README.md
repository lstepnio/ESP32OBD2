...
The Device screen can choose a `.egauge-dev-update` ZIP from Android's system document picker. The importer requires exactly `metadata.json` and `firmware.bin`, bounds the image to the 3 MiB OTA slot, checks board tag, protocol, SHA-256 and ESP app descriptor, and verifies the P-256 signature against the pinned development public key. The development bundle can be made with `tools/sign_dev_update.py --bundle`. **Install development update** sends sequential chunks over the authenticated owner link, checks accepted offsets, asks the gauge to verify and activate, then reads the running partition, valid state and ELF hash after reboot. A [live update](../docs/development/update-live-validation.md), a [changed-build retry after process interruption](../docs/development/update-recovery-validation.md), and an aborted-trial bootloader fallback were observed. The private development key stays outside Git and the APK. The transfer now runs in ViewModel scope and keeps the screen awake while the app is foreground. Process death requires a fresh attempt. Android rollback reporting, power-loss recovery, sustained background delivery, and production signing remain unverified, so the public OTA flag stays false.
