...
Design offers **Refresh saved configuration** and compares that verified readback with the currently selected phone profile. It shows the profile ID, primary PID, renderer, source, and each coolant alert limit or dwell setting side by side, with counts for matches, differences, and unavailable fields. Draft edits update the comparison locally. Unknown or unsupported document fields remain unavailable rather than being treated as matches. The saved side is the last verified snapshot, so refresh before relying on it after changes elsewhere. A comparison does not write to the gauge or prove that the vehicle responds to any PID. The experimental sender always writes numeric pages; other renderer choices stay in the phone preview.

## Signed development update import

The Device screen can choose a `.egauge-dev-update` ZIP from Android's system document picker. The importer requires exactly `metadata.json` and `firmware.bin`, bounds the image to the 3 MiB OTA slot, checks board tag, protocol, SHA-256 and ESP app descriptor, and verifies the P-256 signature against the pinned development public key. The development bundle can be made with `tools/sign_dev_update.py --bundle`. **Install development update** sends sequential chunks over the authenticated owner link, checks accepted offsets, asks the gauge to verify and activate, then reads the running partition, valid state and ELF hash after reboot. A [live update](../docs/development/update-live-validation.md), a [changed-build retry after process interruption](../docs/development/update-recovery-validation.md), a gauge-reboot retry, and an aborted-trial bootloader fallback with Android reporting were observed. The private development key stays outside Git and the APK. The transfer now runs in ViewModel scope and keeps the screen awake while the app is foreground. Process death or a gauge reboot requires a fresh attempt. Power-loss recovery, sustained background delivery, and production signing remain unverified, so the public OTA flag stays false.
