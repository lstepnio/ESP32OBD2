...
This document records experimental code on `feat/owned-gauge-control`, not a released general configuration contract. The bonded owner must have an encrypted, authenticated BLE connection. All writes use existing characteristic `6f1a0002`; reads use `6f1a0003`. This avoids a GATT database change for the paired Pixel. The Android app now contains an explicit numeric ECM profile sender. Two phone transfers and post-reboot active hash readbacks succeeded, as recorded in [hardware evidence](../development/numeric-config-transfer-validation.md). Public `configWrite` and `ota` remain `false`; the separate `experimentalNumericConfig` flag identifies this restricted path. No vehicle session or update has been observed.
