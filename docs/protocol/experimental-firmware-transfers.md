...
This document records experimental code on `feat/owned-gauge-control`, not a released general configuration contract. The bonded owner must have an encrypted, authenticated BLE connection. All writes use existing characteristic `6f1a0002`; reads use `6f1a0003`. This avoids a GATT database change for the paired Pixel. The Android app now contains an explicit numeric ECM profile sender. One phone transfer and post-reboot active hash readback succeeded, as recorded in [hardware evidence](../development/numeric-config-transfer-validation.md). Public `configWrite` and `ota` remain `false`; the separate `experimentalNumericConfig` flag identifies this restricted path. No vehicle session or update has been observed.
...
The current executable subset is one ECM source using headerless functional Mode 01 replies, one-byte PID identifiers, one numeric value per numeric page (up to five pages), and the decoder's first 1..4 payload bytes. It requires metric units, brightness 80, reduced motion false, and alert snooze zero because those settings have no runtime handler yet. The schema's `7E8` response ID is constrained at compile time but cannot be confirmed from a headerless adapter reply; no ECU attribution claim should be made from this path. Mode 22, physical routing, TCM sources, dual pages, other renderers and ECU-specific responses cannot be committed yet. A full schema-valid document may therefore return result 6 at COMMIT. The active generation is loaded at boot; firmware falls back to its built-in readings if a selected generation cannot compile. Polling uses per-definition intervals. Configured thresholds run locally with hysteresis, entry and clear dwell, and a warning or critical badge; the remaining attention/acknowledgment design is pending. When a custom document is active, the public legacy quick-selection and saved-state flags become false because those operations represent only the built-in profile.
...
Owner opcode `30` with a four-byte sequence switches the protected state read to a 32-byte diagnostic snapshot, version `05`. Flag bits at byte 1 indicate fresh MIL evidence (bit 0), MIL on (bit 1), and fresh Mode 03/07/0A category evidence (bits 2/3/4). Byte 2 is the reported Mode 01 PID 01 DTC count. Bytes 3..5 count nonzero DTC pairs in confirmed/pending/permanent replies. Bytes 6..11 are the first raw two-byte DTC in each category, big endian; zero means none. Little-endian observation timestamps at 12, 16 and 20, MIL timestamp at 24, and current monotonic milliseconds at 28 let the caller calculate age. The firmware rejects malformed, ambiguous or multi-responder ELM text. Read-only diagnostics do not authorize Mode 04 clearing.
