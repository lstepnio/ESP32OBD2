...
| Configuration | Owner-only transfer and atomic two-slot commit compile an executable Mode 01 subset. The firmware was flashed and boots with the previous saved rotation | Android Apply, live transfer, full document features, power interruption and configuration trial rollback |
| Diagnostics and alerts | Firmware includes periodic MIL/count and single-responder Mode 03 read, and local threshold dwell/hysteresis badges for an active document | Adapter evidence, full code categories/history, acknowledgment, safe code clearing |
