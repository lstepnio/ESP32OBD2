...
| Updates | The 16 MB layout has two 3 MiB app slots and a rollback-enabled bootloader. A paired Pixel installed a signed development image, then a changed build after a forced app interruption and fresh transfer. The gauge reported the expected ELF hash and valid state after both; revision 2 configuration remained active | Failed-trial rollback, power-loss recovery, production release key and downgrade policy |
