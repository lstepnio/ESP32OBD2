...
| M2: configuration vertical slice | Native Android project started: demo UI, local draft, read-only capabilities; association, read/edit/apply one page, versioned durable config remain | Restart phone/gauge; revision/hash reconcile; interrupted apply retains old config; one real supported reading configured end to end |
